      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-6">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex gap-4 p-4 rounded-xl bg-dark/50 border border-white/5 hover:border-white/10 transition-colors">
                  <div className={`w-2 h-full rounded-full min-h-[40px] ${announcement.type === 'Important' ? 'bg-red-500' : 'bg-primary'}`} />
                  <div>
                    <h4 className="font-semibold">{announcement.title}</h4>
                    <p className="text-sm text-gray-400 mt-1">{announcement.date}</p>
                  </div>
                </div>
              ))}
            </div>
            <button className="w-full mt-4 py-3 text-sm text-gray-400 hover:text-white transition-colors flex items-center justify-center gap-2">
        ...
        <div className="space-y-6">
          <div className="bg-gradient-to-br from-primary/20 to-accent/20 border border-primary/30 rounded-3xl p-6 relative overflow-hidden group cursor-pointer">
            ...
            <p className="text-sm text-gray-300 mb-6">Create a team or join an existing one to participate.</p>
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-6">
            ...
            <ul className="space-y-3">
              <li><Link href="#" className="text-sm text-gray-400 hover:text-primary transition-colors flex items-center justify-between">Submission Guidelines <ChevronRight size={14} /></Link></li>
              <li><Link href="#" className="text-sm text-gray-400 hover:text-primary transition-colors flex items-center justify-between">Join Discord Server <ChevronRight size={14} /></Link></li>
              <li><Link href="#" className="text-sm text-gray-400 hover:text-primary transition-colors flex items-center justify-between">Contact Support <ChevronRight size={14} /></Link></li>
            </ul>
          </div>
        </div>
      </div>
